 ...
 *
 */
public abstract class MTextIconAlignBase extends MTextAlign {

	protected PImage[] bicon = null;
	protected int iconW = 0, iconH = 0;
	protected MAlign iconAlignH = MAlign.RIGHT, iconAlignV = MAlign.MIDDLE;
	protected int siX, siY;


	public MTextIconAlignBase(PApplet theApplet, float p0, float p1, float p2, float p3) {
	 ...
	 */
	public void setIcon(String fname, int nbrImages, MAlign horz, MAlign vert){
		PImage iconImage = MImageManager.loadImage(winApp, fname);
	 ...
	 * @param vert TOP, MIDDLE, BOTTOM
	 */
	public void setIcon(PImage icon, int nbrImages, MAlign horz, MAlign vert){
		if(icon != null){
			if(nbrImages == 3)
				bicon = MImageManager.makeTiles1D(winApp, icon, nbrImages, 1);
			else {
				bicon = new PImage[3];
				PImage[] temp = MImageManager.makeTiles1D(winApp, icon, nbrImages, 1);
			...
			if(horz != null && horz.isHorzAlign() && horz != MAlign.CENTER){ 
				iconAlignH = horz;
			}
	 ...
	 */
	public void setIconAlign(MAlign horz, MAlign vert){
		if(iconW != 0){
			if(horz != null && horz.isHorzAlign() && horz != MAlign.CENTER){
				iconAlignH = horz;
			}
