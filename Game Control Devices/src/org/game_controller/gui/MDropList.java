 ...
 *
 */
public class MDropList extends MTextBase {
	 ...
	 * @param p3
	 */
	public MDropList(PApplet theApplet, float p0, float p1, float p2, float p3) {
	 ...
	 */
	public MDropList(PApplet theApplet, float p0, float p1, float p2, float p3, int dropListMaxSize) {
		...
		vsb = new MScrollbar(theApplet, 0, 0, height - itemHeight, 10);
		vsb.addEventHandler(this, "vsbEventHandler");
		vsb.setAutoHide(true);
		vsb.setVisible(false);

		buttonWidth = 10;
		showList = new MButton(theApplet, 0, 0, buttonWidth, itemHeight, ":");
		...
		createEventHandler(M4P.sketchApplet, "handleDropListEvents",
				new Class<?>[]{ MDropList.class, MEvent.class }, 
	 ...
	 * is for internal library use only.
	 */
	public void buttonShowListHandler(MButton button, MEvent event){
		if(expanded){
			loseFocus(null);
			vsb.setVisible(false);
			expanded = false;
