...
import org.game_controller.gui.MHotSpot.HSalpha;
 ...
 *
 */
public class MImageToggleButton extends MAbstractControl {
	 ...
	 */
	public MImageToggleButton(PApplet theApplet, float p0, float p1){
	 ...
	 */
	public MImageToggleButton(PApplet theApplet, float p0, float p1, String offPicture, int nbrCols){
	 ...
	 * @param nbrRows number of tiles vertically
	 */
	public MImageToggleButton(PApplet theApplet, float p0, float p1, String offPicture, int nbrCols, int nbrRows){
	 ...
	 * @param nbrCols number of tiles horizontally
	 */
	public MImageToggleButton(PApplet theApplet, float p0, float p1, String offPicture, String overPicture, int nbrCols){
	 ...
	 * @param nbrRows number of tiles vertically
	 */
	public MImageToggleButton(PApplet theApplet, float p0, float p1, String offPicture, String overPicture, int nbrCols, int nbrRows){
		super(theApplet, p0, p1, 0, 0);
		// Attempt to get off-control image data
		PImage temp = null;
		if(nbrCols < 1 || nbrRows < 1 || offPicture == null || null == (temp = MImageManager.loadImage(winApp, offPicture))){
			// Invalid data use default
			nbrStates = 2;
			if(toggle == null)
				toggle = MImageManager.loadImage(winApp, TOGGLE);
			offImage = MImageManager.makeTiles1D(winApp, toggle, 2, 1);
		}
		else {
			// Off-control image data valid
			nbrStates = nbrCols * nbrRows;
			offImage = MImageManager.makeTiles1D(winApp, temp, nbrCols, nbrRows);
			// Now check for over-control image data
			if(overPicture != null && null != (temp = MImageManager.loadImage(winApp, overPicture))){
				overImage = MImageManager.makeTiles1D(winApp, temp, nbrCols, nbrRows);
			}
		}
		...
		createEventHandler(M4P.sketchApplet, "handleToggleButtonEvents",
				new Class<?>[]{ MImageToggleButton.class, MEvent.class }, 
