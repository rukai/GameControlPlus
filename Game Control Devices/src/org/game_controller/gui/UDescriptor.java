...
public class UDescriptor extends UBase {

	MTextField txfKey;
	MTextField txfDescription;

	UBaseInput source = null;
	int conID = 0;

	public UDescriptor(UControlConfigWindow ccw, float x, float y) {
		this(ccw, "", "", x, y);
	}

	public UDescriptor(UControlConfigWindow ccw, String key, String desc, float x, float y) {
		super(ccw, x ,y, 1);
		...
		txfKey = new MTextField(app, x + HGAP, y + ccw.textfield_gap, klen, fieldHeight, M4P.SCROLLBARS_NONE);
		txfKey.setFont(ccw.font);
		txfKey.setLocalColorScheme(MCScheme.GOLD_SCHEME);
		txfKey.setDefaultText("Key name");
		txfKey.setText(key);
		txfKey.addEventHandler(this, "key_set");
		ccw.tabManager.addControl(txfKey);

		txfDescription = new MTextField(app, x + 3 * HGAP + klen, y + HGAP/2, dlen, fieldHeight, M4P.SCROLLBARS_NONE);
		txfDescription.setFont(ccw.font);
		txfDescription.setLocalColorScheme(MCScheme.GREEN_SCHEME);
	...
	public void key_set(MTextField source, MEvent event) {
	...
	public void description_set(MTextField source, MEvent event) {
		if(event == MEvent.ENTERED || event == MEvent.LOST_FOCUS){
			if(source.getText().indexOf(SEPARATOR) >= 0)
				source.setText(source.getText().replaceAll(SEPARATOR, " "));
		}
