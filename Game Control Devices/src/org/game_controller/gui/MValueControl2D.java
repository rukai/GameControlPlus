 ...
 *
 */
public abstract class MValueControl2D extends MAbstractControl {
	...
	public MValueControl2D(PApplet theApplet, float p0, float p1, float p2, float p3) {
	...
	public void pre(){
		if(Math.abs(parametricTargetX - parametricPosX) > epsilon || Math.abs(parametricTargetY - parametricPosY) > epsilon){
			...
			bufferInvalid = true;
			if(Math.abs(parametricTargetX - parametricPosX) > epsilon || Math.abs(parametricTargetY - parametricPosY) > epsilon){
				fireEvent(this, MEvent.VALUE_CHANGING);
			}
			else {
				parametricPosX = parametricTargetX;
				parametricPosY = parametricTargetY;
				fireEvent(this, MEvent.VALUE_STEADY);
			}
		}
	}
