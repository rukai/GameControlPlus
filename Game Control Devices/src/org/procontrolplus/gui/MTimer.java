 ...
 *
 */
public class MTimer implements MConstantsInternal {
	 ...
	 */
	public MTimer(PApplet theApplet, Object obj, String methodName, int delay){
		app = theApplet;
	...
	public MTimer(PApplet theApplet, Object obj, String methodName, int delay, int initDelay){
		app = theApplet;
		createEventHandler(obj, methodName);
		// If we have something to handle the event then create the Timer
		if(eventHandlerObject != null){
	...
	protected void createEventHandler(Object handlerObj, String methodName){
		try{
			eventHandlerMethod = handlerObj.getClass().getMethod(methodName, new Class<?>[] { MTimer.class } );
			eventHandlerObject = handlerObj;
			eventHandlerMethodName = methodName;			
		} catch (Exception e) {
			MMessenger.message(NONEXISTANT, new Object[] {this, methodName, new Class<?>[] { MTimer.class }});
